...
def render_clip(project, clip, index, out_path, target, level_db=-16.0):
    ...
    video_filter = ("[0:v]" + pre
                    + fit_filter(view_w, view_h, target["width"], target["height"]) + "[v]")
    fade = min(0.07, seconds / 6.0)
    audio_filter = (
        "[0:a]aresample=%s,loudnorm=I=%.1f:TP=-1.5:LRA=11,"
        "afade=t=in:st=0:d=%.3f,afade=t=out:st=%.3f:d=%.3f[a]"
        % (encode["sample_rate"], level_db, fade, max(seconds - fade, 0.0), fade))
...
def render_timeline(project, out_path, target, captions="full", corrections=()):
    ...
    if captions in ("", "none"):
        _finish(joined, out_path, target, total, None)
    else:
        ...
        ass = subtitles.write_ass(cues, work / "captions.ass",
                                  ...
                                  baseline=int(target["height"] * 0.555) if captions == "centre"
                                  else int(target["height"] * 0.86),
                                  glow=captions == "centre")
        ...
        _finish(joined, out_path, target, total, ass)
        print("  %d cues, sidecar %s" % (len(cues), out_path.with_suffix(".srt").name))

    return out_path


def _finish(joined, out_path, target, total, ass):
    """The last pass: captions if there are any, the target's loudness, faststart."""
    encode = target["encode"]
    video = "[0:v]subtitles='%s'[v]" % subtitles.ass_path_for_filter(ass) if ass \
        else "[0:v]null[v]"
    if target["loudness"] is not None:
        audio = "[0:a]loudnorm=I=%.1f:TP=-1.5:LRA=11[a]" % target["loudness"]
    else:
        audio = "[0:a]anull[a]"
